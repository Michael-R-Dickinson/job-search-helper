...
const TASK_SWITCH_INTERVAL = 1500
const MAX_TASKS_ON_SCREEN = 3

const TailoredResumeLoading: React.FC = () => {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      setCompletedTasks((prev) => {
        ...
        return updated.length > MAX_TASKS_ON_SCREEN
          ? updated.slice(updated.length - MAX_TASKS_ON_SCREEN)
          : updated
      })
      // Move to next
      setCurrentIndex((prev) => (prev + 1 < COMPLETING_TASKS.length ? prev + 1 : -1))
    }, TASK_SWITCH_INTERVAL)
...
export default TailoredResumeLoading
